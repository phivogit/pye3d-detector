...
class GazeMapper:
    ...
    def projectPoints(self, object_points, rvec=None, tvec=None):
        ...
        if rvec is None and tvec is None:
            # If no rotation/translation is provided, assume points are in world coordinates
            rvec = cv2.Rodrigues(self.rotation_matrix)[0]
            tvec = self.eye_camera_to_world_matrix[:3, 3]
    ...
    def map_gaze(self, result_3d):
        ...
        image_point = self.front_camera_intrinsics.projectPoints(
            gaze_point[np.newaxis].reshape(-1, 1, 3),  # Reshape gaze_point for projectPoints
            self.rotation_vector,                       # Use the rotation vector defined in constructor
            self.translation_vector                     # Use the translation vector defined in constructor
        )
