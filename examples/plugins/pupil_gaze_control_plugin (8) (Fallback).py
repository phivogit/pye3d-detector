...
class Gaze_Control(Plugin):
    ...
    def detect_apriltags(self, frame):
        ...
        if len(detections) != 4:
            logger.warning(f"Detected {len(detections)} tags, need 4; using fallback")
            # Use fixed screen coordinates as fallback
            ...
            # Assume a default camera-space quadrilateral for fallback (scaled to frame size)
            scale_factors = np.array([frame_width / 1920, frame_height / 1080], dtype=np.float32)
            camera_tag_corners = self.fixed_tag_corners * scale_factors[None, :]
            logger.debug(f"Fallback - camera_tag_corners: {camera_tag_corners}")
            logger.debug(f"Fallback - normalized_corners: {normalized_corners}")
            return cv2.getPerspectiveTransform(camera_tag_corners, normalized_corners)
        ...
        if len(tag_corners) != 4:
            logger.warning(f"Found {len(tag_corners)} valid tag corners; using fallback")
            normalized_corners = self.fixed_tag_corners.copy()
            normalized_corners[:, 0] /= self.screen_width
            normalized_corners[:, 1] /= self.screen_height
            scale_factors = np.array([frame_width / 1920, frame_height / 1080], dtype=np.float32)
    ...
    def recent_events(self, events):
        ...
        if frame is not None:
            ...
            if transform is not None:
                ...
                logger.info("Screen mapping updated with AprilTags or fallback")
        
        # Process gaze data with throttling
