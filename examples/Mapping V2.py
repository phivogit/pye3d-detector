...
class FrontCameraIntrinsics:
    ...
    def projectPoints(self, object_points, rvec=None, tvec=None, use_distortion=True):
        ...
        :param use_distortion: Whether to use distortion coefficients
        :return: Projected 2D points
        """
        # Save input dimensionality to reshape output correctly
        input_dim = object_points.ndim
        
        # Reshape object points to expected format
        object_points = np.array(object_points).reshape((1, -1, 3))
        
        # Handle optional rotation vector
        if rvec is None:
            rvec = np.zeros(3).reshape(1, 1, 3)
        else:
            rvec = np.array(rvec).reshape(1, 1, 3)
        
        # Handle optional translation vector
        if tvec is None:
            tvec = np.zeros(3).reshape(1, 1, 3)
        else:
            tvec = np.array(tvec).reshape(1, 1, 3)
        
        # Determine which distortion coefficients to use
        _dist_coeffs = self.dist_coeffs if use_distortion else np.zeros_like(self.dist_coeffs)
        
        # Project points using OpenCV
        image_points, _ = cv2.projectPoints(
            object_points, rvec, tvec, self.camera_matrix, _dist_coeffs
        )
        
        # Reshape output based on input dimensionality
        if input_dim == 2:
...
class GazeMapper:
    ...
    def map_gaze(self, result_3d):
        ...
        image_point = self.front_camera_intrinsics.projectPoints(
            gaze_point[np.newaxis],  # Using the new method with better reshaping
